...
func (f *Filter) CheckHost(host string, port int) Verdict {
	h := strings.ToLower(strings.TrimSuffix(host, "."))
	if hardDenyHosts[h] {
		return f.log(h, port, Verdict{Deny, "hard-deny metadata host"})
	}
	if ip, err := netip.ParseAddr(h); err == nil && isLinkLocal(ip) {
		return f.log(h, port, Verdict{Deny, "hard-deny link-local address"})
	}
	if v := f.checkRules(h); v != nil {
	...
	return f.log(h, port, Verdict{Deny, "default deny"})
}
