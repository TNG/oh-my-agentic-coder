...
- **Linux**: bubblewrap provides mount-namespace filesystem isolation (replacing nono's Landlock fs rules); a re-exec'd `omac sandbox stage2` inside bwrap applies **Landlock TCP rules** (handled access: `bind_tcp` + `connect_tcp`, ABI ≥ 4) and a **seccomp-BPF filter** denying `socket(SOCK_DGRAM|SOCK_RAW)` for AF_INET/AF_INET6 (blocking UDP/ICMP), then execs the inner command. The host network namespace is kept (no `--unshare-net`) so loopback access to the proxy and facade works. `--new-session` is passed to bwrap to prevent TIOCSTI injection; SIGWINCH is relayed by the supervisor to preserve terminal resize.
- Kernels without Landlock ABI v4 (< 6.7): fail closed with a clear error. Profile/CLI escape hatch `network.enforcement: "env-only"` runs with proxy env vars but no kernel guarantee, printing a prominent warning. (seccomp-notify supervision is not used — large complexity, shrinking audience; the narrower socket-level seccomp filter above covers the datagram gap.)
...
```jsonc
{
  ...
  "filesystem": {
    ...
    "deny":  [".env", "*.key", "~/.aws"],         // mask within grants: bare name = any granted dir (incl. cwd); path = exact
...
- User deny entries (`filesystem.deny` / `--deny`) resolve to concrete paths during grant resolution (basename globs are expanded by walking the granted trees and the workdir) and then reuse the protected-path masking above, so no backend-specific code is needed.
