...
In `filtered` mode the sandbox SHALL deny unauthorized network access from the child at the kernel level. On macOS this SHALL be enforced via Seatbelt network rules (`(deny network*)` plus targeted allows, including an explicit allow for granted Unix socket paths). On Linux this SHALL be enforced via two complementary mechanisms applied by a stage-2 re-exec inside bwrap: Landlock TCP rules (`connect_tcp`/`bind_tcp`, ABI ≥ 4) restricting TCP, and a seccomp-BPF filter denying `socket(AF_INET|AF_INET6, SOCK_DGRAM|SOCK_RAW)` to block UDP and ICMP egress not expressible via Landlock. Allowed traffic is: outbound TCP to the supervisor proxy on loopback, the configured port openings (`listen_port`, `allow_tcp_connect`, `open_port`), granted Unix sockets, and loopback DNS resolution facilities required by the platform.
