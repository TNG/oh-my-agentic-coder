...
The default cache scope SHALL be `workdir` so that each workdir gets its own isolated directory. The `global` scope (one directory shared across all workdirs) remains available via explicit configuration but is not the default. A note on the security ceiling: concurrent sessions within the same workdir still share a writable cache; the workdir default prevents cross-project cache poisoning, not same-project concurrent-session poisoning.
