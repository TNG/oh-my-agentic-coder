...
|---|---|---|
| `--no-sandbox` | false | Run the harness without any sandbox. Removes all isolation — debug use only. Combine with `--inner bash` for an unsandboxed shell in the omac environment. |
...
| `--cache-scope <scope>` | workdir | Which cache to use for the agent's package downloads (npm, pip, cargo, etc.). `global` shares one cache across all your projects, `config` shares it across projects using the same config file, `workdir` gives each project its own isolated cache. See [Cache](../advanced/cache.md). |
| `--profile-path <path>` | — | Use this sandbox grants profile for the session, overriding `sandbox.profile_name`. The path must be inside `~/.config/omac/sandbox-profiles/` or `<workdir>/.omac/`; a relative path is anchored to the workdir. Symlinks are refused. A path into the project layer is its own first approval and is pinned per file. See [Sharing a profile across a team](../configuration.md#sharing-a-profile-across-a-team). |
| `--accept-project-config` | false | Approve the project-local `.omac` sandbox configuration (required once on first use) and re-approve it after a change; without it, omac refuses to start when the project layer contributes configuration. |
...
| `--accept-project-config` | false | Approve / re-approve the project-local `.omac` sandbox configuration. Same as on `omac start`. |
